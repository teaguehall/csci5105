...
int checksum_file(const char* folder, FileInfo* file_info_to_modify)
{
    ...
    strcat(file_path, "/");
    strcat(file_path, file_info_to_modify->name);
    ...
    if(file == NULL)
    {
        fprintf(stderr, "Checksum failed. Could not open file \"%s\": %s\n", file_path, strerror(errno));
        return -1;
    }

    // calculate file size  
    fseek(file, 0, SEEK_END);
    file_info_to_modify->size = ftell(file);
    fseek(file, 0, SEEK_SET);

    // read file byte-by-byte to calculate checksum
